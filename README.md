...
```bash
npm install react react-dom clsx tailwind-merge tailwindcss@^4
```
...
This library requires Tailwind CSS v4 and the DSK base styles for proper rendering.
이 라이브러리는 Tailwind CSS v4와 DSK 기본 스타일이 필요합니다.

```css
/* globals.css */
@import 'tailwindcss';
@import '@happy2870/dsk/css';
```

The base CSS includes:
기본 CSS에 포함된 내용:

- Design token colors (C-Primary, C-Gray-*, point-* etc.) / 디자인 토큰 컬러
- Typography utilities (text-body-14-b, text-title-18-b etc.) / 타이포그래피 유틸리티
- Font face declarations (Pretendard, GmarketSans via CDN) / 폰트 선언
- Component shadows and animations / 컴포넌트 그림자 및 애니메이션

> If your project already uses `@gsn/config/css/base.css`, you don't need to import `@happy2870/dsk/css` — the tokens are identical.
> 프로젝트에서 이미 `@gsn/config/css/base.css`를 사용 중이라면 `@happy2870/dsk/css`를 별도로 import할 필요 없습니다 — 토큰이 동일합니다.
...
- **Tailwind CSS v4** for styling / 스타일링
